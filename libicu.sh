...
if $host_cross; then
    [ -d "$SRCDIR/icu4c/source/build_${build_arch}" ] || $ROOTDIR/libicu.sh ${build_arch} --both --bin
    CFG="--with-cross-build=$SRCDIR/icu4c/source/build_${build_arch}"
fi

BUILD_DIR="$SRCDIR/icu4c/source/build_${arch}"
