...
. tcutils.sh
CFG="--with-libiconv-prefix=$LIBSDIR --disable-decoder --disable-debug --with-pic=1"
dbld=$SRCDIR
loadToolchain
test $arch != x86_64-linux-gnu && CFG="--host=${arch} --with-sysroot=${SYSROOT} $CFG"
[[ $arch = *mingw32* ]] && CFG="$CFG --enable-expopt=full"

export CPPFLAGS="-I$LIBSDIR/include" LDFLAGS="-L$LIBSDIR/lib" LIBS="-liconv -lm" PKG_CONFIG_PATH=$LIBSDIR/lib/pkgconfig

vrs=$(svn log ${svn}/tags --limit 1 | grep 'tag' | sed "s/tag \(.*\) release/\1/")

# make shared executable so
[[ "$CBN" = "$cb1" ]] && [[ "$CSH" = "$cs1" ]] && cb1="$cb1 --enable-dynamic-frontends"
