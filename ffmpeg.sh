...
sudo apt install build-essential dialog >/dev/null 2>&1
echo
...
extraOpts(){
  case $1 in
    ...
    *iec61883 )      setArg libiec61883 iec61883 ;;
    *ilbc )          setArg libilbc libilbc ;;
    *jack )          setArg libjack libjack ;;
    *klvanc )        setArg libklvanc klvanc ;;
    ...
    *opus )          setArg libopus 'libogg vorbis opus' ;;
...
case $arch in
  aarch64-linux-android ) setNDKToolchain
    CFG="--cpu=armv8-a --arch=aarch64 --target-os=android --enable-neon --enable-asm --enable-inline-asm --cc=$CC --cxx=$CXX --cross-prefix=${CROSS_PREFIX} --sysroot=$SYSROOT $CFG";;
  arm-linux-androideabi ) setNDKToolchain
    CFG="--cpu=armv7-a --arch=armv7-a --target-os=android --enable-neon --enable-asm --enable-inline-asm --cc=$CC --cxx=$CXX --cross-prefix=${CROSS_PREFIX} --sysroot=$SYSROOT $CFG";;
  i686-linux-android ) setNDKToolchain
    CFG="--cpu=i686 --arch=i686 --target-os=android --disable-neon --disable-asm --disable-inline-asm --cc=$CC --cxx=$CXX --cross-prefix=${CROSS_PREFIX} --sysroot=$SYSROOT $CFG";;
  x86_64-linux-android ) setNDKToolchain
    CFG="--cpu=x86_64 --arch=x86_64 --target-os=android --disable-neon --enable-asm --enable-inline-asm --cc=$CC --cxx=$CXX --cross-prefix=${CROSS_PREFIX} --sysroot=$SYSROOT $CFG";;
  *-w64-mingw32 ) setMinGWToolchain
    CFG="--arch=${ABI} --target-os=mingw32 --cross-prefix=${CROSS_PREFIX} $CFG";;
  x86_64-linux-gnu ) setGnuToolchain
    CFG="--arch=amd64 $CFG";;
  xi686-linux-gnu ) setGnuToolchain
    CFG="--arch=x86 $CFG";;
esac
