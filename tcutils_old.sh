...
if test -z "$INIT"; then
  ...
  test -z $(which autoconf) && \
    aptInstallBr automake autoconf autogen autopoint libtool-bin m4
...
installNdk(){
  NDK_URL="https://dl.google.com/android/repository/android-ndk-${1}-linux.zip"
  pushd ${ANDROID_HOME} >/dev/null
  download ${NDK_URL}
  mv android-ndk-* android-ndk
  popd >/dev/null
...
while [ $1 ];do
  case $1 in
    ...
    --updateall ) showBanner && aptUpdate && exit 0;;
    --update )
    ...
    --shared ) bshared=1;;
    --noshared ) bshared=0;;
    --installNdk) shift && installNdk $1 && exit 0;;
    --bin ) bbin=1;;
    --nobin ) bbin=0;;
    --cmake ) cfg='cm';;
    --advanced ) advcfg=1;;
